...
// Rechtliche Seiten auf der Webseite (in der App unter Info & Einstellungen verlinkt)
const String privacyPolicyUrl = 'https://duschletta.me/countr/datenschutz.html';
const String impressumUrl = 'https://duschletta.me/impressum.html';
...
class AppTranslations {
  static final Map<String, Map<String, String>> _values = {
    'de': {
      ...
      'resume_title': 'Spiel fortsetzen?', 'resume_body': 'Du hast ein unbeendetes Spiel. Möchtest du dort weitermachen, wo du aufgehört hast?',
      'resume_yes': 'FORTSETZEN', 'resume_no': 'NEUES SPIEL',
      ...
      'privacy_title': 'Datenschutz', 'impressum_title': 'Impressum & Kontakt', 'address': 'Sportlerweg 10\n8360 Eschlikon',
    ...
    'en': {
      ...
      'resume_title': 'Continue Game?', 'resume_body': 'You have an unfinished game. Continue where you left off?',
      'resume_yes': 'CONTINUE', 'resume_no': 'NEW GAME',
      ...
      'privacy_title': 'Privacy Policy', 'impressum_title': 'Imprint & Contact', 'address': 'Sportlerweg 10\n8360 Eschlikon\nSwitzerland',
    ...
    'fr': {
      ...
      'resume_title': 'Reprendre la partie ?', 'resume_body': 'Vous avez une partie en cours. Voulez-vous reprendre où vous en étiez ?',
      'resume_yes': 'REPRENDRE', 'resume_no': 'NOUVELLE PARTIE',
      ...
      'privacy_title': 'Politique de confidentialité', 'impressum_title': 'Mentions légales', 'address': 'Sportlerweg 10\n8360 Eschlikon\nSuisse',
...
class GameData {
  ...
  final IconData icon;
  final Widget Function(BuildContext, Color, {bool resume})? pageBuilder;
...
List<GameData> get globalGameList => [
  GameData(translationKey: 'game_title_std', icon: Icons.onetwothree, category: GameCategory.utility, minPlayers: 1, maxPlayers: null,
      pageBuilder: (c, color, {resume = false}) => StandardCounterGame(themeColor: color, resume: resume)),
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    _loadPreferences();
    WidgetsBinding.instance.addPostFrameCallback((_) => _checkResumableGame());
  }

  Future<void> _checkResumableGame() async {
    final gameId = await GamePersistence.getResumableGameId();
    if (gameId == null || !mounted) return;

    GameData? game;
    for (final g in _getGamesList()) {
      if (g.id == gameId && g.pageBuilder != null) {
        game = g;
        break;
      }
    }
    if (game == null) {
      await GamePersistence.clear(gameId);
      return;
    }

    final resumeGame = game;
    final shouldContinue = await showDialog<bool>(
      context: context,
      barrierDismissible: false,
      builder: (context) => AlertDialog(
        backgroundColor: darkSurface,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15), side: BorderSide(color: activeColor)),
        title: Text(AppTranslations.get('resume_title'), style: const TextStyle(color: Colors.white)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(resumeGame.title, style: TextStyle(color: activeColor, fontWeight: FontWeight.bold, fontSize: 16)),
            const SizedBox(height: 8),
            Text(AppTranslations.get('resume_body'), style: const TextStyle(color: Colors.white70)),
          ],
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
            child: Text(AppTranslations.get('resume_no'), style: const TextStyle(color: Colors.grey)),
          ),
          TextButton(
            onPressed: () => Navigator.pop(context, true),
            child: Text(AppTranslations.get('resume_yes'), style: TextStyle(color: activeColor, fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
    ...
    if (shouldContinue == true) {
      Navigator.of(context).push(
        MaterialPageRoute(builder: (context) => resumeGame.pageBuilder!(context, activeColor, resume: true)),
      );
    } else {
      await GamePersistence.clear(gameId);
    }
  ...
  List<GameData> _getGamesList() {
    return [
      ...
      GameData(translationKey: 'game_title_darts', icon: Icons.track_changes, category: GameCategory.other, minPlayers: 1, maxPlayers: 8,
          pageBuilder: (c, color, {resume = false}) => DartsGame(themeColor: color, resume: resume)),
...
class _InfoScreenState extends State<InfoScreen> {
          ...
          body: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                // --- RECHTLICHES (Pflicht für App Store / Play Store) ---
                const Divider(color: Colors.white24, height: 40),
                _legalLink(
                  key: const ValueKey('privacy_link'),
                  icon: Icons.privacy_tip_outlined,
                  label: AppTranslations.get('privacy_title'),
                  url: privacyPolicyUrl,
                ),
                const SizedBox(height: 8),
                _legalLink(
                  key: const ValueKey('impressum_link'),
                  icon: Icons.info_outline,
                  label: AppTranslations.get('impressum_title'),
                  url: impressumUrl,
                ),
  ...
  // Öffnet Datenschutz / Impressum im Browser
  Widget _legalLink({required Key key, required IconData icon, required String label, required String url}) {
    return Material(
      color: darkSurface,
      borderRadius: BorderRadius.circular(15),
      child: InkWell(
        ...
        onTap: () async {
          try {
            ...
            // Kein Browser verfügbar - nichts zu tun
          }
        },
        child: Container(
          ...
          child: Row(
            children: [
              Icon(icon, color: brandYellow, size: 22),
              const SizedBox(width: 15),
